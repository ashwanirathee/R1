...
class SegmentorNode(Node):
    def __init__(self):
        super().__init__("segmentor_node")
        self.bridge = CvBridge()

        self.declare_parameter("camera_uid", 10)
        self.camera_uid = int(self.get_parameter("camera_uid").value)

        self.declare_parameter("enable_task_2_segmentation_tracking", False)
        self.enable_tracking = as_bool(
            self.get_parameter("enable_task_2_segmentation_tracking").value
        )

        self.declare_parameter("task_2_segmentation_output_mode", "events")
        self.output_mode = (
            self.get_parameter("task_2_segmentation_output_mode").value
        ).lower()
        if self.output_mode not in ("events", "overlay", "both"):
            self.get_logger().warning(
                "Invalid task_2_segmentation_output_mode "
                f"'{self.output_mode}', falling back to 'events'."
            )
            self.output_mode = "events"

        self.declare_parameter("segmentor_model", "yolo11n-seg.pt")
        self.model_name = self.get_parameter("segmentor_model").value
        ...
        self.event_pub = self.create_publisher(String, "/segmentor/events", 10)
        self.overlay_pub = self.create_publisher(
            CompressedImage,
            "/segmentor/overlay_image/compressed",
            10,
        )

        self.model = YOLO(self.model_name)
        self.get_logger().info(
            f"Segmentor node started for camera {self.camera_uid}."
        )
    ...
    def image_callback(self, msg):
        ...
        for result in results:
            ...
            mask_polygons = masks.xyn if masks is not None else []
            ...
            for index, (xywhn, cls, conf, track_id) in enumerate(
                ...
                if track_id is not None:
                    segment["track_id"] = int(track_id.item())
