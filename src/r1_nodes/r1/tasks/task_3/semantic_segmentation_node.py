    ...
    def __init__(self):
        ...
        if self.output_mode not in ("events", "overlay", "both"):
            self.get_logger().warning(
                "Invalid task_3_semantic_segmentation_output_mode "
        ...
        self.event_pub = self.create_publisher(
            String,
            "/semantic_segmentation/events",
            10,
        )
        self.overlay_pub = self.create_publisher(
            CompressedImage,
            "/semantic_segmentation/overlay_image/compressed",
            10,
        )

        self.model = YOLO(self.model_name)
        self.get_logger().info(
            f"Semantic segmentation node started for camera {self.camera_uid}."
        )
        ...
        for result in results:
            ...
            for index, (xywhn, cls, conf, track_id) in enumerate(
                ...
                if track_id is not None:
                    semantic_segment["track_id"] = int(track_id.item())
